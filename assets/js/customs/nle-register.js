...
function cardCheck(id,name){
	...
	if(selected > 0){
		...
		$('#v4-services-active').removeClass('timeline-box-active');
		$('#timeline-container').addClass('timeline-continue');	
	}else{
		$("#v4-product").fadeOut(500);
		$("#v4-basic-info").fadeOut(500);
		$('#v4-services-active').addClass('timeline-box-active');
		$('#timeline-container').removeClass('timeline-continue');
		$('#timeline-container').removeClass('timeline-continue-last');
	}
...
function cardCheckRadio(id,name){
    ...
    if(!$("input[id="+id+"]").is(":checked")){
		$("#v4-basic-info").fadeOut(500);
		$('#v4-basic-info-active').removeClass('timeline-box-active');
		$('#v4-product-active').addClass('timeline-box-active');
		$('#timeline-container').removeClass('timeline-continue-last').addClass('timeline-continue');
		
        $(".card").css("border-color","#e9e9ef");
        $("#card_"+id).css("border-color","#e9e9ef");
        $("input[id="+id+"]:checked")[0].checked = false;
    }else{
		$("#v4-basic-info").fadeIn(500);	
		$('#v4-basic-info-active').addClass('timeline-box-active');
		$('#v4-product-active').removeClass('timeline-box-active');
		$('#timeline-container').removeClass('timeline-continue').addClass('timeline-continue-last');

...
<form class="" action="<?= (!empty($collections) ? $collections['action'] : site_url('profile/setCallBackAddressBillingStore')); ?>" autocomplete="off" method="post" id="v4-billing-info-edit" name="v4-billing-info-edit">
    <?= (!empty($collections) ? '<input type="hidden" name="addressId" value="' . $collections['attribute']['addressId']. '" wajib="yes" readonly>' : ''); ?>
    <div class="pin-address">
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 t24-l24">
                <input type="text" name="addressDetailPin" id="v4-company-address-selector" class="form-control input-v4 input-v4-pl-40 w-100" placeholder="Write street Name/Building Name/Housing Area" name="addressDetailPinPoint">
                <div class="icon">
        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div id="v4-company-address-pin" style="margin: 0 auto; width: 100%; height: 243px;" name="addressPinLatLong"></div>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 t24-l24">
                <input type="text" name="addressDetaillatlang" id="v4-company-address-latlang" class="form-control input-v4 input-v4-pl-40 w-100" placeholder="-">
                <div class="icon">
        ...
        <div class="row">
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="form">
                    ...
                    <?= form_dropdown('provinceId', $provinceId, (!empty($collections) ? $collections['attribute']['provinceId'] : ''), 'class="form-control input-v4-mb-8 input-v4-pl-12 w-100 bes-select" id="provinceId" data-url="' . site_url('autocomplete/getCityByProvince'). '" data-target="cityId"'); ?>
                </div>
            </div>
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="form">
                    <label for="">City</label>
                    <?= form_dropdown('cityId', $cityId, (!empty($collections) ? $collections['attribute']['cityId'] : ''), 'class="form-control input-v4-mb-8 input-v4-pl-12 w-100 bes-select" id="cityId" data-url="' . site_url('autocomplete/getDistrictByCity'). '" data-target="districtId"'); ?>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="form">
                    ...
                    <?= form_dropdown('districtId', $districtId, (!empty($collections) ? $collections['attribute']['districtId'] : ''), 'class="form-control input-v4-mb-8 input-v4-pl-12 w-100 bes-select" id="districtId" data-url="' . site_url('autocomplete/getAreaIdByDistrict'). '" data-target="areaId"'); ?>
                </div>
            </div>
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="form">
                    <label for="">Area</label>
                    <?= form_dropdown('areaId', $areaId, (!empty($collections) ? $collections['attribute']['areaId'] : ''), 'class="form-control input-v4-mb-8 input-v4-pl-12 w-100 bes-select" id="areaId"'); ?>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="form-floating">
                    <textarea class="form-control input-v4 input-v4-mb-8 input-v4-pl-12 w-100" id="addressDetailManual" placeholder="Address" wajib="yes" name="addressDetailManual" style="height: 80px; resize: none;"><?= (!empty($collections) ? $collections['attribute']['addressDetail']: ''); ?></textarea>
                    <label for="addressDetailManual">Address</label>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="form-floating">
                    <input type="text" class="form-control input-v4 input-v4-mb-8 input-v4-pl-12 w-100" id="postalCode" placeholder="Postal Code" wajib="yes" name="postalCode" <?= (!empty($collections) ? 'value="' . $collections['attribute']['postalCode'] . '"' : ''); ?>>
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-12">
            <div class="wrapper-buttons-dialog wrapper-w-loader" id="<?= rand(); ?>">
                ...
                <button class="btn-container-dialog btn-dialog-submit" id="<?= rand(); ?>" onclick="v4DoPost('v4-billing-info-edit', $(this), 'v4-list-user-active'); return false;"><?= (!empty($collections) ? 'Update' : 'Save'); ?></button>
            </div>
        </div>
    </div>
</form>
<script>
    function autocompleteWithEnter(map) {
        const searchInput = document.getElementById("v4-company-address-selector");
        const options = {};
        ...
        google.maps.event.addListener(autocomplete, "place_changed", function () {
            ...
            if (typeof place.address_components !== "undefined") {
                ...
                $("#v4-company-address-latlang").val(userLocation.lat + ', ' + userLocation.lng);
            }
        ...
        function geocodePosition(pos) {
            ...
            geocoder.geocode({latLng: pos}, function(results, status){
                if (status == google.maps.GeocoderStatus.OK) {
                    $("#v4-company-address-selector").val(results[0].formatted_address);
                    $("#v4-company-address-latlang").val(results[0].geometry.location.lat() + ', ' + results[0].geometry.location.lng());
                }  else  {
                    console.log(status);
                }
            });
    ...
    function initialize() {
        ...
        const map = new google.maps.Map(
            document.getElementById("v4-company-address-pin"),
            mapOptions
        );
